...
//call this function wherever any library function call is made,
// this will print the cause of the error and exit the program with int value 1(non-zero value for failed)
...
int editorRowRxToCursorX(erow *row, int rx) {
    int cur_rx = 0;
    int cx;
...
void abAppend(struct abuf *ab, const char *string, int stringLength) {
    char *new = realloc(ab->b, ab->len + stringLength);

    if(new == NULL) return;
    memcpy(&new[ab->len], string, stringLength);
    ab->b =  new;
    ab->len += stringLength;
}
...
void editorDrawStatusBar(struct abuf *ab) {
    ...
    E.filename ? E.filename : "[No Name]",( E.dirty ? "*" : "" ),E.cursorY + 1, E.cursorX + 1);
    // int rlen = snprintf(rstatus, sizeof(rstatus), "%d:%d",
    //           E.cursorY + 1, E.cursorX + 1);
...
void editorRefreshScreen() {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (E.cursorY - E.rowOffset) + 1, (E.rx - E.colOffset) + 1);
